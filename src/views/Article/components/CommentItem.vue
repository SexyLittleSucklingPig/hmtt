<template>
  <van-cell>
    ...
    <div slot="label">
      ...
      <p>
        <span style="margin-right: 10px">{{ item.pubdate | datefromat }}</span>
        <van-button
          size="mini"
          type="default"
...
<script>
import { addLikes, delLikes } from '@/api/comments'
export default {
  props: {
    item: {
      type: Object,
  ...
  methods: {
    async onClick () {
      ...
      if (this.item.is_liking) {
        try {
          await delLikes(this.item.com_id)
        ...
        try {
          await addLikes(this.item.com_id)
          this.item.like_count++
